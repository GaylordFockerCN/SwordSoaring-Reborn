...
public class PatchedBabylonRandomReplaceableLayer<E extends BabylonEntity, T extends AbstractArtifactSpiritPatch<E>, M extends EntityModel<E>, AM extends AnimatedMesh> extends PatchedLayer<E, T, M, RenderLayer<E, M>, AM> {
    ...
    public static void renderPortal(PoseStack poseStack, float alpha, float r, float g, float b, ResourceLocation portalTexture, MultiBufferSource buffer, float frame) {
        ...
        consumer.vertex(pMatrix, -1, -1, 0).color(r, g, b, alpha).uv(0, 0.05F * frame - 0.05F).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LightTexture.FULL_BRIGHT).normal(normal, 1, 1, 1).endVertex();
        consumer.vertex(pMatrix, 1, -1, 0).color(r, g, b, alpha).uv(1, 0.05F * frame - 0.05F).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LightTexture.FULL_BRIGHT).normal(normal, 1, 1, 1).endVertex();
        consumer.vertex(pMatrix, 1, 1, 0).color(r, g, b, alpha).uv(1, 0.05F * frame).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LightTexture.FULL_BRIGHT).normal(normal, 1, 1, 1).endVertex();
        consumer.vertex(pMatrix, -1, 1, 0).color(r, g, b, alpha).uv(0, 0.05F * frame).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LightTexture.FULL_BRIGHT).normal(normal, 1, 1, 1).endVertex();
